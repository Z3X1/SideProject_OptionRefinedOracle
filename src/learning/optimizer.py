...
"""
[LAYER: L5 LEARNING]
職責：五層迭代優化：L1 梯度下降（週期聚類×時間衰減雙權重）/ L2 信號方向準確率 / L3 Regime 分層 / L4 貝葉斯先驗（leave-one-out）/ L5 收斂偵測（凍結/解凍）。污染樣本剔除、雙重統計門檻（樣本數+週期數）。
"""
import json, os, math
from datetime import datetime, timezone
from config.settings import (DEFAULT_WEIGHTS, DEFAULT_REGIME_WEIGHTS,
                             UFT_KEYS as KEYS, DECAY_HALF_LIFE, BAYES_PRIOR_STR)
from learning.ledger import load_log, save_log
